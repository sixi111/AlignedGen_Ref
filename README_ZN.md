...
**1) 显存不足怎么办？**  
先减少 prompt 数量、降低分辨率、减少步数；必要时再启用 offload（如果你已接入）。

**2) 为什么风格不稳定或不明显？**  
优先检查 `--reference_prompt` 是否过于空泛；其次微调 `--style_lambda`。

**3) 如何做可复现对比实验？**  
固定 `--seed` 与 `--cache_seed`，并记录 `--reference_kv_precompute_mode` 与步数配置。
