...
| **预计算** | `reference_kv_precompute_mode`：`appendix_a` 为论文附录 A（噪声–参考 latent 插值）；`denoise` 为纯噪声 + `scheduler.step` 逐步去噪并每步缓存 KV（验证用，与主去噪轨迹一致）。 |
...
- **`--cache_seed`**：附录 A 中为插值噪声种子；**`denoise`** 模式下为初始 packed latent 的高斯噪声种子。
- **`--reference_kv_precompute_mode`**：`appendix_a`（默认）或 `denoise`。
